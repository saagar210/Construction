...
import { OUTCOME_SEVERITY_LABELS, INJURY_TYPE_LABELS, INCIDENT_STATUS_LABELS } from '../../lib/constants';
import type { IncidentStatus } from '../../lib/types';

export function IncidentDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { currentIncident: incident, attachments, loading, loadIncident, updateIncident, deleteIncident, clearCurrent } = useIncidentStore();
  ...
  if (loading || !incident) {
    return <p className="text-gray-500">{loading ? 'Loading...' : 'Incident not found'}</p>;
  }
  ...
  const field = (label: string, value: string | number | null | undefined | boolean) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${
              ...
              incident.status === 'in_review' ? 'bg-blue-100 text-blue-800' :
              'bg-green-100 text-green-800'
            }`}>
              {INCIDENT_STATUS_LABELS[incident.status]}
            </span>
          </div>
          <p className="text-gray-500 text-sm">{incident.incident_date} - {incident.is_privacy_case ? 'Privacy Case' : incident.employee_name}</p>
        </div>
        <div className="flex gap-2">
          <Link to={`/incidents/${incident.id}/rca`}
            className="bg-safety-blue text-white px-3 py-1.5 rounded text-sm hover:bg-blue-600">
            Root Cause Analysis
          </Link>
          <select value={incident.status} onChange={e => handleStatusChange(e.target.value as IncidentStatus)}
      ...
      <section className="bg-white rounded-lg shadow p-6">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {field('Employee', incident.is_privacy_case ? 'Privacy Case' : incident.employee_name)}
        ...
        <div className="mt-4">
          ...
          <dd className="text-sm mt-1 whitespace-pre-wrap">{incident.description}</dd>
        </div>
      </section>
      ...
      <section className="bg-white rounded-lg shadow p-6">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {field('Severity', OUTCOME_SEVERITY_LABELS[incident.outcome_severity])}
          {field('Injury Type', INJURY_TYPE_LABELS[incident.injury_illness_type])}
            ...
            {attachments.map(a => (
              <li key={a.id} className="flex items-center justify-between border rounded p-2 text-sm">
                <div>
                  ...
                  <span className="text-gray-400 ml-2">{a.file_type}</span>
                </div>
              </li>
            ))}
          </ul>
      ...
      <section className="border border-red-200 rounded-lg p-6">
        ...
        {confirmDelete ? (
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setConfirmDelete(false)} className="border px-3 py-1.5 rounded text-sm">
              Cancel
            </button>
          </div>
        ) : (
          <button onClick={() => setConfirmDelete(true)} className="text-red-600 text-sm hover:underline">
