  ...
  const loadData = async () => {
    ...
    try {
      const [templateData, instanceData] = await Promise.all([
        ...
        invoke<JsaInstance[]>('list_jsa_instances', { establishmentId: activeEstablishment.id }),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">Create and track JSAs for jobs and tasks.</p>
      </div>

      <form onSubmit={handleCreate} className="bg-white rounded-lg shadow p-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        <select value={templateId} onChange={(e) => setTemplateId(e.target.value ? Number(e.target.value) : '')} className="border rounded px-3 py-2">
          <option value="">No template</option>
          {templates.map((template) => (
            <option key={template.id} value={template.id}>{template.title}</option>
          ))}
        </select>
        <input className="border rounded px-3 py-2" placeholder="Job name" value={jobName} onChange={(e) => setJobName(e.target.value)} required />
        <input className="border rounded px-3 py-2" type="date" value={jobDate} onChange={(e) => setJobDate(e.target.value)} required />
        <input className="border rounded px-3 py-2" placeholder="Prepared by" value={preparedBy} onChange={(e) => setPreparedBy(e.target.value)} required />
        <div className="md:col-span-4">
          <button className="bg-safety-orange text-white px-4 py-2 rounded" type="submit">Create JSA</button>
        </div>
      </form>
              ...
              {instances.map((instance) => (
                <tr key={instance.id}>
                  ...
                  <td className="px-4 py-2">
                    <span className="px-2 py-0.5 rounded bg-blue-100 text-blue-800 text-xs">{instance.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
