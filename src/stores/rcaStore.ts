...
import type {
  RcaSession, FiveWhysStep, FishboneCategory, FishboneCause, CorrectiveAction,
} from '../lib/types';
...
interface RcaState {
  ...
  createSession: (incidentId: number, method: 'five_whys' | 'fishbone') => Promise<RcaSession>;
  ...
  addFiveWhysStep: (sessionId: number, stepNumber: number, question: string, answer: string) => Promise<void>;
  updateFiveWhysStep: (id: number, question: string, answer: string) => Promise<void>;

  loadFishboneCategories: (sessionId: number) => Promise<void>;
  addFishboneCategory: (sessionId: number, category: string, sortOrder?: number) => Promise<FishboneCategory>;
  addFishboneCause: (categoryId: number, causeText: string, isRootCause?: boolean) => Promise<FishboneCause>;
  updateFishboneCause: (id: number, causeText?: string, isRootCause?: boolean) => Promise<void>;
  deleteFishboneCause: (id: number) => Promise<void>;

  loadCorrectiveActions: (incidentId: number) => Promise<void>;
  createCorrectiveAction: (incidentId: number, description: string, assignedTo?: string, dueDate?: string, rcaSessionId?: number) => Promise<void>;
  updateCorrectiveAction: (id: number, data: Record<string, unknown>) => Promise<void>;
...
export const useRcaStore = create<RcaState>((set, get) => ({
  ...
  loadSessions: async (incidentId) => {
    set({ loading: true });
    try {
      const sessions = await invoke<RcaSession[]>('list_rca_sessions', { incidentId });
  ...
  loadFiveWhysSteps: async (sessionId) => {
    const steps = await invoke<FiveWhysStep[]>('list_five_whys_steps', { rcaSessionId: sessionId });
    set({ fiveWhysSteps: steps });
  },

  addFiveWhysStep: async (sessionId, stepNumber, question, answer) => {
    await invoke('add_five_whys_step', {
      data: { rca_session_id: sessionId, step_number: stepNumber, question, answer },
    });
  ...
  loadFishboneCategories: async (sessionId) => {
    const categories = await invoke<FishboneCategory[]>('list_fishbone_categories', { rcaSessionId: sessionId });
  ...
  addFishboneCause: async (categoryId, causeText, isRootCause) => {
    const cause = await invoke<FishboneCause>('add_fishbone_cause', {
      data: { category_id: categoryId, cause_text: causeText, is_root_cause: isRootCause },
    });
    const session = get().currentSession;
  ...
  loadCorrectiveActions: async (incidentId) => {
    const actions = await invoke<CorrectiveAction[]>('list_corrective_actions', { incidentId });
    set({ correctiveActions: actions });
  },

  createCorrectiveAction: async (incidentId, description, assignedTo, dueDate, rcaSessionId) => {
  ...
  updateCorrectiveAction: async (id, data) => {
    ...
    // Reload from the current incident context
  },
